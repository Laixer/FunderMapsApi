...
- **Identifiers: BAG and CBS codes are the keys.** `geocoder.building.id` / `address.id` (and the neighborhood → state chain) still carry internal `gfm-` surrogates; their retirement is Laixer/FunderMapsWorker#158. Until then: every `building_id` column in the database is the BAG pand id = `building.external_id`, so join buildings on `external_id`, never on `id` (API #179 was exactly that mistake). Address ids on the wire are BAG nummeraanduidingen (`address.external_id`); accept a `gfm-` address id only where a client echoes one the API handed out, and look each kind of id up in its own column (`lib/geocoder-id.ts` `fromIdentifier`), never with an OR across columns.
