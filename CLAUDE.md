...
- **Identifiers: BAG and CBS codes are the keys.** Addresses are done: `geocoder.address` is keyed by the BAG nummeraanduiding (`external_id`), the `gfm-` address id no longer exists, and every address id stored or on the wire is that nummeraanduiding (`id` and `external_id` carry the same value where a response still has both). Still `gfm-`: `geocoder.building.id`, `residence.id` and the neighborhood → state chain; their retirement is Laixer/FunderMapsWorker#158. Until then: every `building_id` column in the database is the BAG pand id = `building.external_id`, so join buildings on `external_id`, never on `id` (API #179 was exactly that mistake), and look each kind of id up in its own column (`lib/geocoder-id.ts` `fromIdentifier`), never with an OR across columns.
- **`geocoder.building_active` view removal**. The view has 15+ dependents through a chain of maplayer views, statistics matviews, and Worker-managed objects. Single migration of 200+ lines of SQL; explicitly "not a small win, defer."
